...
SynkroniqSDK.shared.initialize(apiKey: "YOUR_API_KEY", platform: "ios")
...
SynkroniqSDK.shared.getServiceCategories(requestBody: userContext) { result in
    guard case .success(let categoriesJson) = result else { return }
    print("Categories:", categoriesJson)

    // 3. Create a ticket under service category 1
    let ticketBody = """
    {
        "uid": 20,
        "email": "user@example.com",
    ...
    """
    SynkroniqSDK.shared.createTicket(requestBody: ticketBody) { ticketResult in
        guard case .success(let ticketJson) = ticketResult else { return }
        print("New ticket:", ticketJson)

        // 4. Add a follow-up article
        let articleBody = """
        {
            "uid": 20,
            "email": "user@example.com",
        ...
        """
        SynkroniqSDK.shared.addTicketArticle(requestBody: articleBody) { articleResult in
